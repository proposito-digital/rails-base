...
RSpec.describe PolicyAliasProbeTwoPolicy, type: :policy do
  let(:actor) { build(:user) }
  let(:record) { :policy_alias_probe_two }

  describe ".scope" do
    it "returns all users in scope" do
      user_a = create(:user, email_address: "probe_two_one@example.com")
      user_b = create(:user, email_address: "probe_two_two@example.com")
