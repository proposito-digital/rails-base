...
RSpec.describe PolicyAliasProbeThreePolicy, type: :policy do
  let(:actor) { build(:user) }
  let(:record) { :policy_alias_probe_three }

  describe ".scope" do
    it "returns all users in scope" do
      user_a = create(:user, email_address: "probe_three_one@example.com")
      user_b = create(:user, email_address: "probe_three_two@example.com")
